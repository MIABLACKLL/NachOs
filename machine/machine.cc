...
Machine::Machine(bool debug)
{
    ...
    pageTable = NULL;
#endif
    globalPageTable = new TranslationEntry[NumPhysPages];
    for(int i=0;i<NumPhysPages;i++)
    {
	globalPageTable[i].virtualPage = i;
	globalPageTable[i].physicalPage = i;
	globalPageTable[i].valid = TRUE;
	globalPageTable[i].use = FALSE;
	globalPageTable[i].dirty = FALSE;
	globalPageTable[i].readOnly = FALSE;
	globalPageTable[i].recUsed = time(NULL);
...
int Machine::FindPage()
{
    //sleep(0.001);
    for(int i=0;i<NumPhysPages;i++)
    {
	if(globalPageTable[i].valid)
	 {
		globalPageTable[i].valid = FALSE;
		globalPageTable[i].use = TRUE;
		globalPageTable[i].recUsed = clock();
		//cout<<globalPageTable[i].recUsed<<endl;
		return i;
	 }
    }
    RaiseException(PageFaultException);
    return -1;
